...
/**
 * A factory for creating FakeHandleProvider objects.
 * Note that the FakeProviderFactory is not a generic "fake" factory - it is specific for DOIs.
 */
@AutoService(PidProviderFactory.class)
public class FakeHandleProviderFactory implements PidProviderFactory  {
    ...
    public PidProvider createPidProvider(String providerId) {
        ...
        if (!providerType.equals(FakeHandleProvider.TYPE)) {
            // Being asked to create a non-FakeHandle provider
            return null;
        }
        ...
        return new FakeHandleProvider(providerId, providerLabel, providerAuthority, providerShoulder, identifierGenerationStyle,
                datafilePidFormat, managedList, excludedList, index, independentHandleService, handleAuthHandle, path,
                passphrase);
    }

    public String getType() {
        return FakeHandleProvider.TYPE;
    }
}
